const generatedAccountsFilePath = 'cypress/fixtures/generatedAccounts.json';

const getAccount = (email = '') => {
	return new Cypress.Promise((resolve, reject) => {
		cy.task('readFileMaybe', generatedAccountsFilePath).then((text) => {
			let generatedAccounts =
				text == null ? {} : JSON.parse(text).generatedAccounts;
			let returnAccount;

			if (email === '') {
				//if no email provided get the latest added account
